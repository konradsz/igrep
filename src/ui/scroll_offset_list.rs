    ...
    L: Iterator<Item = Text<'b>>,
{
    ...
    pub fn scroll_offset(mut self, scroll_offset: ScrollOffset) -> List<'b, L> {
    ...
    L: Iterator<Item = Text<'b>>,
{
    ...
    fn render(mut self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        let (items1, items2) = self.items.tee();
        let list_size = items1.count();
        ...
        if let Some(selected) = selected {
            ...
            if selected >= list_height + state.offset - self.scroll_offset.bottom
                && selected < list_size - self.scroll_offset.bottom
            {
                state.offset += 1;
            }
        }

        for (i, item) in items2
            .skip(state.offset)
            .enumerate()
